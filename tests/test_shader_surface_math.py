...
from ..ydr.shader_materials import (
    _spec_map_is_packed, _spec_map_is_diffuse, _specular_params, _is_parallax_pom, _is_displacement,
    _is_terrain_parallax, IGNORE_SPEC_MAP_VALUES_SHADERS,
    VEHICLE_SPECULAR_CONSTANTS, TILE_UV_PARAMS,
)
...
# Shaders built with USE_DIFFUSE_AS_SPEC in the game source.
DIFFUSE_AS_SPEC_SHADERS = {
    "normal_diffspec", "normal_diffspec_detail", "normal_diffspec_detail_dpm",
    "normal_diffspec_detail_dpm_tnt", "normal_diffspec_detail_tnt", "normal_diffspec_tnt",
}


def test_diffuse_as_spec_classification_matches_game_shader_families():
    actual = {s.base_name for s in ShaderManager._shaders.values() if _spec_map_is_diffuse(s)}

    assert actual == DIFFUSE_AS_SPEC_SHADERS

    for shader in ShaderManager._shaders.values():
        if not _spec_map_is_diffuse(shader):
            continue
        # No SpecSampler: the diffuse map is the spec map. No specularFalloffMult: the game reads
        # the falloff from specSamp.w, which is 0 on this path, so the term is stripped.
        assert "SpecSampler" not in shader.parameter_map
        assert "specularFalloffMult" not in shader.parameter_map


def game_spec_desaturate_overlay(a, base):
    """The Overlay blend in ``CalculateSpecularContribution`` under SPECULAR_DIFFUSE."""
    return 2.0 * a * base if a < 0.5 else 1.0 - 2.0 * (1.0 - a) * (1.0 - base)


def blender_overlay_mix_node(a, base, fac=1.0):
    """Blender's Mix Color node, blend type OVERLAY, in_a=a, in_b=base."""
    facm = 1.0 - fac
    if a < 0.5:
        return a * (facm + 2.0 * fac * base)
    return 1.0 - (facm + 2.0 * fac * (1.0 - base)) * (1.0 - a)


@pytest.mark.parametrize("a", [0.0, 0.1, 0.25, 0.49999, 0.5, 0.5001, 0.75, 1.0])
@pytest.mark.parametrize("base", [0.0, 0.0625, 0.3, 0.625, 1.0])
def test_diffuse_as_spec_overlay_matches_blender_mix_node(a, base):
    assert blender_overlay_mix_node(a, base) == pytest.approx(game_spec_desaturate_overlay(a, base), abs=1e-9)
...
def blinn_roughness(exponent):
    """The conversion in ``gta_blinn_roughness_expr``."""
    blinn = 3.0 * exponent + 555.0 * max(0.0, exponent - 500.0)
    return (2.0 / (blinn + 2.0)) ** 0.25


def test_ignore_spec_map_values_shaders_actually_have_a_spec_map():
    # The override only means anything for shaders that ship a SpecSampler; otherwise it is dead.
    for base_name in IGNORE_SPEC_MAP_VALUES_SHADERS:
        shader = next((s for s in ShaderManager._shaders.values() if s.base_name == base_name), None)

        assert shader is not None, f"{base_name} is not a real shader"
        assert "SpecSampler" in shader.parameter_map
        assert shader.filename in ShaderManager.veh_paints


# VEHCONST_SPECULAR2_* baked into the paint shaders that do not expose them as parameters.
VEHCONST_SPECULAR2_FALLOFF = 15.0
VEHCONST_SPECULAR2_INTENSITY = 0.75


@pytest.mark.parametrize("shader_name", sorted(ShaderManager.veh_paints))
def test_vehicle_paint_second_specular_is_a_wide_sheen_not_a_mirror(shader_name):
    shader = ShaderManager._shaders[shader_name]

    def param_or(name, default):
        return shader.parameter_map[name].x if name in shader.parameter_map else default

    falloff = param_or("specular2Factor", VEHCONST_SPECULAR2_FALLOFF)
    intensity = param_or("specular2ColorIntensity", VEHCONST_SPECULAR2_INTENSITY)

    assert blinn_roughness(falloff) > 0.3
    assert intensity >= 0.0


@pytest.mark.parametrize("shader_name", sorted(ShaderManager.veh_paints))
def test_vehicle_paint_second_specular_is_masked_by_the_spec_map(shader_name):
    shader = ShaderManager._shaders[shader_name]

    assert "SpecSampler" in shader.parameter_map
    assert "specMapIntMask" in shader.parameter_map


TREES_ALPHA_TO_COVERAGE = 2.0


@pytest.mark.parametrize("shader_name", sorted(
    n for n, s in ShaderManager._shaders.items() if s.base_name.startswith("trees")
))
def test_trees_alpha_test_keeps_fully_opaque_texels(shader_name):
    shader = ShaderManager._shaders[shader_name]
    if "AlphaTest" not in shader.parameter_map:
        pytest.skip("no alpha test")

    scale = shader.parameter_map["AlphaScale"].x * TREES_ALPHA_TO_COVERAGE

    assert 1.0 * scale > shader.parameter_map["AlphaTest"].x


def test_tile_uv_params_belong_to_their_sampler():
    for tex_name, param_name in TILE_UV_PARAMS.items():
        users = [s for s in ShaderManager._shaders.values() if param_name in s.parameter_map]

        assert users, f"{param_name} is not used by any shader"
        for shader in users:
            assert tex_name in shader.parameter_map, f"{shader.filename} has {param_name} but no {tex_name}"
