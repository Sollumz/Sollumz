...
def _spec_map_is_diffuse(shader: ShaderDef) -> bool:
    """Shaders built with USE_DIFFUSE_AS_SPEC derive the specular map from the diffuse texture."""
    return "SpecDesaturateIntensity" in shader.parameter_map
...
IGNORE_SPEC_MAP_VALUES_SHADERS = frozenset({"vehicle_paint1", "vehicle_paint1_enveff"})
...
def gta_specular_expr(shader: ShaderDef, spec_tex_name: Optional[str]):
    from ..shared.shader_expr.builtins import tex, uv, vec, param, maxf, saturate, mix_color
    ...
    if shader.base_name in IGNORE_SPEC_MAP_VALUES_SHADERS:
        spec_tex_name = None

    if _spec_map_is_diffuse(shader):
        diffuse_name = "DiffuseSampler"
        diffuse = tex(diffuse_name, uv(shader.uv_maps.get(diffuse_name, 0))).color
        desat_int = param("SpecDesaturateIntensity")
        desat_exp = param("SpecDesaturateExponent")
        spec_samp = mix_color(
            vec(diffuse.dot(desat_int.vec), diffuse.dot(desat_exp.vec), 0.0),
            vec(desat_int.w, desat_exp.w, 0.0),
            1.0,
            ColorBlend.OVERLAY,
        )
        mask = param("specMapIntMask")
        intensity = spec_samp.dot(vec(mask.x, mask.y, mask.z)) * intensity_mult
        exponent = 0.0
    elif shader.base_name == "emissive_speclum":
        diffuse_name = "DiffuseSampler"
        diffuse = tex(diffuse_name, uv(shader.uv_maps.get(diffuse_name, 0))).color
        spec_mult = 1.0 - diffuse.dot(vec(0.2125, 0.7154, 0.0721))  # LumFactors
        intensity = spec_mult * intensity_mult
        exponent = spec_mult * falloff_mult
    elif spec_tex_name is None:
    ...
    return saturate(intensity), gta_blinn_roughness_expr(exponent), gta_fresnel_ior_expr(fresnel)


def gta_blinn_roughness_expr(exponent: "expr.Floaty") -> "expr.Floaty":
    from ..shared.shader_expr.builtins import maxf

    blinn_exponent = 3.0 * exponent + 555.0 * maxf(exponent - 500.0, 0.0)
    return (2.0 / (blinn_exponent + 2.0)) ** 0.25
...
def create_decal_nodes(b: ShaderBuilder, texture, decalflag):
    ...
    if decalflag == 0:  # cutout
        ...
        if (
            ...
            (alpha_test := try_get_node(node_tree, "AlphaTest")) and
            isinstance(alpha_test, SzShaderNodeParameter)
        ):
            scaled_alpha = node_tree.nodes.new("ShaderNodeMath")
            scaled_alpha.operation = "MULTIPLY"
            links.new(texture.outputs["Alpha"], scaled_alpha.inputs[0])
            if (alpha_scale := try_get_node(node_tree, "AlphaScale")) is not None:
                links.new(alpha_scale.outputs["X"], scaled_alpha.inputs[1])
            else:
                scaled_alpha.inputs[1].default_value = 1.0

            if b.shader.base_name.startswith("trees"):
                coverage = node_tree.nodes.new("ShaderNodeMath")
                coverage.operation = "MULTIPLY"
                coverage.inputs[1].default_value = 2.0
                links.new(scaled_alpha.outputs["Value"], coverage.inputs[0])
                scaled_alpha = coverage

            alpha_gt = node_tree.nodes.new("ShaderNodeMath")
            alpha_gt.operation = "GREATER_THAN"
            links.new(scaled_alpha.outputs["Value"], alpha_gt.inputs[0])
            links.new(alpha_test.outputs["X"], alpha_gt.inputs[1])
...
TILE_UV_PARAMS = {
    "DiffuseSampler": "DiffuseTexTileUV",
    "SpecSampler": "specTexTileUV",
}


def link_tile_uvs(b: ShaderBuilder):
    node_tree = b.node_tree

    for tex_name, param_name in TILE_UV_PARAMS.items():
        tex_node = try_get_node(node_tree, tex_name)
        tile_node = try_get_node(node_tree, param_name)
        if tex_node is None or tile_node is None or not tex_node.inputs[0].is_linked:
            continue

        scale = node_tree.nodes.new("ShaderNodeVectorMath")
        scale.operation = "SCALE"
        node_tree.links.new(tex_node.inputs[0].links[0].from_socket, scale.inputs[0])
        node_tree.links.new(tile_node.outputs["X"], scale.inputs["Scale"])
