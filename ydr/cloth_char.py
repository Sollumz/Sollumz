...
def cloth_char_export(drawable_obj: Object, drawable_xml: Drawable, materials: list[Material]) -> Optional[CharacterCloth]:
    print(f"cloth_char_export({drawable_obj})")
    ...
    char_cloth.bounds = create_composite_xml(cloth_bounds_obj)
    # TODO(cloth): check that there are only capsule bounds
...
def cloth_char_export_dictionary(dwd_obj: Object) -> Optional[ClothDictionary]:
    print(f"cloth_char_export_dictionary({dwd_obj})")
    ...
    for child in dwd_obj.children:
        ...
        cloth = cloth_char_export(child, None, [])
        if cloth is None:
            continue
