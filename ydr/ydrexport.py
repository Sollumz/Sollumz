...
def drawable_model_from_object(obj, bones=None, materials=None):
    ...
    if len(geometries_to_join) > 0:
        ...
        for obj in objs:
            ...
            obj.modifiers.new(name = "normals" , type="DATA_TRANSFER")
            obj.modifiers["normals"].object = child
            obj.modifiers["normals"].use_loop_data = True
            obj.modifiers["normals"].data_types_loops = {"CUSTOM_NORMAL"}
            geometry = geometry_from_object(
                obj, materials, bones)
            drawable_model.geometries.append(geometry)
...
def light_from_object(obj, armature_obj=None):
    ...
    light.shadow_blur = obj.data.light_properties.shadow_blur
