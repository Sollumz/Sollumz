...
from ..tools.fragmenthelper import convert_selected_to_fragment, convert_selected_to_window
...
class SOLLUMZ_OT_create_window(SOLLUMZ_OT_base, bpy.types.Operator):
    """Create a sollumz window fragment."""
    bl_idname = "sollumz.createwindow"
    bl_label = f"Create Window Fragment"
    bl_action = "Create A Window Fragment"
    bl_update_view = True
    ...
    def run(self, context):
        aobj = context.active_object
        selected = context.selected_objects
        fragment_type = context.scene.create_window_type
        if fragment_type == SollumType.FRAGVEHICLEWINDOW and len(selected) > 0:
            dobjs = convert_selected_to_window(
                selected, context.scene.use_mesh_name, context.scene.create_seperate_objects, context.scene.create_center_to_selection)
            if context.scene.auto_create_embedded_col:
                cobjs = convert_selected_to_bound(
                    context.selected_objects, use_name=False, multiple=context.scene.create_seperate_objects, bvhs=True, replace_original=False, do_center=False)
            ...
            self.message(
                f"Succesfully converted {', '.join([obj.name for obj in context.selected_objects])} to a {SOLLUMZ_UI_NAMES[SollumType.FRAGVEHICLEWINDOW]}.")
            return True
        else:
            obj = create_empty_object(fragment_type)
            if aobj:
